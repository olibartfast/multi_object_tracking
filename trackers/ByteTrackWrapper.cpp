    ...
    : BaseTracker(config)
{
    ...
    // Note: ByteTrack constructor is (frame_rate, track_buffer, track_thresh, high_thresh, match_thresh)
    // We'll use a default frame_rate of 30
...
std::vector<TrackedObject> ByteTrackWrapper::update(const std::vector<Detection>& detections, const cv::Mat& frame)
{
    ...
    for (const auto& detection : detections) {
        if (config_.classes_to_track.find(detection.label) != config_.classes_to_track.end()) {
            ...
            byte_track::Object obj(rect, detection.label, detection.score);
